...
const FLEET_AUTH_VERSION = 1
const MAX_ENVELOPE_LIFETIME_MS = 5 * 60_000
...
function requireAuth(condition, code, message) {
  if (!condition) throw authError(code, message)
}
...
function canonicalValue(value, depth) {
  ...
  if (typeof value === 'number') {
    requireAuth(Number.isFinite(value), 'invalid_payload', 'Numbers must be finite.')
...
function normalizePublicKeySpki(value) {
  ...
  try {
    ...
    throw authError('invalid_public_key', 'Device public key could not be read.')
  }
  requireAuth(
...
function normalizePrivateKey(privateKey) {
  ...
  try {
    ...
    throw authError('invalid_private_key', 'Device private key could not be read.')
  }
  requireAuth(
    key.asymmetricKeyType === 'ed25519',
    'invalid_private_key',
...
function unsignedEnvelope(input) {
  ...
  requireAuth(
    protocolVersion === FLEET_AUTH_VERSION,
    'unsupported_version',
...
function createSignedDeviceRequest({
  ...
  const signature = sign(null, Buffer.from(canonicalJson(unsigned)), key)
  return {
    ...unsigned,
...
function verifySignedDeviceRequest({
  ...
  requireAuth(
    safeTextEqual(unsigned.payloadDigest, expectedDigest),
    'payload_mismatch',
...
function identityKey(publicKeySpki) {
  return normalizePublicKeySpki(publicKeySpki).key
}
