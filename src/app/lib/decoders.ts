...
export function decodeWorkout(raw: Raw, id: string, userIdFallback?: string): WorkoutSession {
  return {
    ...
    pausedTime: num(raw.pausedTime) ?? 0,
    distance: num(raw.distance) ?? 0,
    elevationGain: num(raw.elevationGain) ?? 0,
    elevationLoss: num(raw.elevationLoss) ?? 0,
    ...
    averagePower: num(raw.averagePower),
    maxPower: num(raw.maxPower),
    averageStrideLength: num(raw.averageStrideLength),
    averageVerticalOscillation: num(raw.averageVerticalOscillation),
    averageGroundContactTime: num(raw.averageGroundContactTime),
    heartRateRecoveryOneMinute: num(raw.heartRateRecoveryOneMinute),
    workoutEffortScore: num(raw.workoutEffortScore),
    isFavorite: bool(raw.isFavorite) ?? false,
    notes: str(raw.notes),
