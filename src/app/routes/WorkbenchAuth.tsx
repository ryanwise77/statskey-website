...
export function WorkbenchAuth() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        // `useAuth` exposes a trimmed AppUser; the ID token comes from the
        // Firebase Auth instance that holds the live session.
        const current = auth.currentUser
        if (!current) throw new Error('No signed-in user for the Workbench session.')
