...
export function NudgeStudio() {
  ...
  async function publish() {
    ...
    const confirmed = window.confirm(`Publish revision ${revision} to StatsKey now?`)
    if (!confirmed) return
    setAction('publish')
    setError(null)
    setNotice(null)
  ...
  async function rollback(target: NudgeRevision) {
    ...
    const confirmedRestore = window.confirm(
      `Restore revision ${target.revision}? This publishes it as a new audited revision.`
    )
  ...
  async function publishWeekNote() {
    ...
    const confirmed = window.confirm(
      `Publish the Week ${week.weekNumber} founder note? This note will appear immediately on the public Olympic Marathon training screen.`
    )
    if (!confirmed) return
