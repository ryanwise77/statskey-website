...
for (const path of publicPaths) {
  test(`${path} advertises the single approved Pro service without stale tiers`, () => {
    const source = read(path)
    assert.match(source, /\$69\.99/)
    assert.match(source, /\$699/)
    assert.doesNotMatch(source, /Pro\+|\$14\.99|\$29\.99|\$99\.99|\$999|\$149\.99|\$199\.99/)
...
test('all translated support offers include one Pro and distinguish independently metered uses', () => {
  let catalog
...
test('actual app uses the unified offer and access adapter while token-pack catalog remains independent', () => {
  assert.match(read('src/app/routes/Profile.tsx'), /PRO_SUBSCRIPTION_OPTIONS\.map/)
  assert.doesNotMatch(read('src/app/routes/Profile.tsx'), /proPlusMonthly|proPlusAnnual/)
  assert.match(read('src/app/components/NutritionFacts.tsx'), /useProAccess\(subscription\)/)
